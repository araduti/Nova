...
<#
.SYNOPSIS
    AmpCloud Bootstrap - WinRE/WinPE loader with HTML Progress UI.
.DESCRIPTION
    ...
    - Drives the HTML Progress UI (Progress/index.html) running in Edge
      kiosk mode via JSON IPC (X:\AmpCloud-Status.json).
...
# ── Assemblies ──────────────────────────────────────────────────────────────
# WinForms and Drawing are still required for interactive modal dialogs
# (WiFi selector, configuration menu, authentication, completion screen)
# and for WinForms timers that drive the non-blocking init state machine.
# The main progress display is handled by the HTML UI in Edge kiosk mode.
...
# sequences into real newlines so labels render them correctly.
...
function Invoke-NetworkTuning {
    ...
    .DESCRIPTION
        ...
        call from a timer tick without freezing the thread.
    #>
...
#region ── Main Form ── Hidden owner window for WinForms message pump ─────────
# The visible progress UI is the HTML page running in Edge kiosk mode
# (Progress/index.html).  This hidden form provides the WinForms message
# pump that keeps timers and modal dialogs (WiFi selector, configuration
# menu, authentication) alive.
...
function Update-Step { param([int]$s) Update-HtmlUi -Step $s }

# ── Live status IPC from AmpCloud.ps1 ──────────────────────────────────────
# AmpCloud.ps1 writes progress to a JSON file; the HTML UI polls it directly.
# This timer is kept only so Bootstrap can detect when AmpCloud signals Done.
...
function Show-CompletionScreen {
    ...
    # Signal the HTML UI that imaging is complete.
    Update-HtmlUi -Message $S.Complete -Done
...
function ProceedToEngine {
    ...
    try {
        ...
        # Run AmpCloud.ps1 in a dedicated process so the event loop
        # stays responsive.
...
function Show-Failure {
    ...
    if (Test-InternetConnectivity) {
        ...
        # Close the hidden form; the -NoExit PowerShell host from
...
# that froze the thread for 20-30 seconds.
#
# States:
    ...
    switch ($script:_initState) {
        ...
        'CHECK' {
            ...
            if (Test-InternetConnectivity) {
                ...
                # Show WiFi selector directly — the HTML UI is the visible
                # progress display, so there is no on-screen button to press.
                $wifiConnected = Show-WiFiSelector
                if ($wifiConnected -and (Test-InternetConnectivity)) {
                    ProceedToEngine
                } else {
                    # Start a background timer to detect a late wired connection.
...
# Launch — the hidden form provides the WinForms message pump while the
# HTML Progress UI (Edge kiosk mode) is the visible display.
