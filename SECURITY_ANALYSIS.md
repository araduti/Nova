...
3. [Component Analysis](#component-analysis)
   ...
   - [Bootstrap.ps1 — Device Code Flow](#bootstrapps1--device-code-flow)   - [Editor (Web UI) — MSAL.js Popup Flow](#editor-web-ui--msaljs-popup-flow)
...
│  PKCE (local  │     (WinPE WinForms    │    (browser, sessionStorage) │
│  browser)     │      dialog)           │                              │
├───────────────┼────────────────────────┼──────────────────────────────┤
│  Opens browser│  Shows one-time code   │  MSAL loginPopup()           │
│  → localhost  │  → operator enters at  │  → Azure AD popup            │
│  listener     │  microsoft.com/device  │  → redirect callback         │
│  captures code│  login on another      │  → sessionStorage cache      │
│  → token      │  device → token poll   │                              │
│  exchange     │                        │                              │
└───────────────┴────────────────────────┴──────────────────────────────┘
```
...
**Orchestrator:** `Invoke-M365Auth`
**OAuth Flow:** Device Code (RFC 8628)
**Environment:** Windows PE (WinForms dialog)

#### Flow Steps

1. **Config fetch** (`Invoke-M365Auth`): Downloads `auth.json` from GitHub over HTTPS
2. **Device code request** (`Invoke-M365DeviceCodeAuth`): POSTs to the `/devicecode` endpoint to obtain `user_code` and `device_code`
3. **WinForms dialog** (`Invoke-M365DeviceCodeAuth`): Displays the `user_code` in a styled dialog with the verification URL
4. **Token polling** (`Invoke-M365DeviceCodeAuth`): Timer-based polling every `interval` seconds; handles `authorization_pending` and `slow_down` responses

#### Security Assessment

| Aspect | Status | Details |
|--------|--------|---------|
| Token storage | ✅ Ephemeral | Tokens are validated for presence but not stored. |
| Device Code Flow | ✅ Standard | Full Device Code Flow with timer polling per RFC 8628. |
| Phishing risk | ⚠️ Inherent | Device Code Flow inherently requires users to visit a URL and enter a code (see Finding F-02). |
...
**Component:** `Bootstrap.ps1`, `Invoke-M365DeviceCodeAuth`
...
**Mitigation:** WinPE does not include a modern browser engine, so Device Code Flow is the appropriate OAuth method for this environment. The Azure AD consent screen always shows the application name, giving the user an opportunity to verify the request.
**Recommendation:** No code change required. Operators should be trained to verify the application name on the Azure AD consent screen matches AmpCloud.
...
| **Device Code phishing** | Attacker displays their own code to an operator | Azure AD consent screen shows app name; operators trained to verify | Low |
