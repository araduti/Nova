...
│  PKCE (local  │     (WinPE standalone  │    (browser, sessionStorage) │
│  browser)     │      Edge browser,    │                              │
│               │      Device Code       │                              │
│               │      fallback)         │                              │
├───────────────┼────────────────────────┼──────────────────────────────┤
│  Opens browser│  Launches msedge.exe   │  MSAL loginPopup()           │
│  → localhost  │  → localhost listener  │  → Azure AD popup            │
│  listener     │  captures redirect     │  → redirect callback         │
...
### Bootstrap.ps1 — Edge Browser (Auth Code + PKCE) with Device Code Fallback

**Primary function:** `Invoke-M365EdgeAuth` (standalone Edge browser with PKCE + localhost listener)
**Fallback function:** `Invoke-M365DeviceCodeAuth` (Device Code Flow)
**Orchestrator:** `Invoke-M365Auth` (tries Edge browser first, falls back to device code)
**OAuth Flow:** Authorization Code with PKCE (primary) / Device Code (fallback)
**Environment:** Windows PE (standalone msedge.exe with Chromium + SwiftShader rendering)

#### Flow Steps (Primary — Edge Browser)

1. **Config fetch** (`Invoke-M365Auth`): Downloads `auth.json` from GitHub over HTTPS
2. **Edge prerequisite check** (`Invoke-M365EdgeAuth`): Verifies `msedge.exe` exists at `X:\WebView2\Edge`
3. **PKCE generation** (`Invoke-M365EdgeAuth`): Generates 32 random bytes → base64url code verifier; SHA-256 hash → code challenge
4. **Localhost HTTP listener** (`Invoke-M365EdgeAuth`): Starts an HTTP listener on a random high port to capture the OAuth redirect
5. **Edge launch** (`Invoke-M365EdgeAuth`): Launches `msedge.exe` with WinPE-safe flags (`--allow-run-as-system`, `--disable-gpu`, `--use-angle=swiftshader`, `--in-process-gpu`, etc.) navigated to Azure AD `/authorize` with PKCE challenge
6. **Redirect capture** (`Invoke-M365EdgeAuth`): The HTTP listener receives the localhost redirect and extracts the authorization code from the query string
7. **Token exchange** (`Invoke-M365EdgeAuth`): POSTs code + code_verifier to the `/token` endpoint; validates `id_token` presence
8. **Fallback** (`Invoke-M365Auth`): If Edge browser is unavailable or fails, transparently falls back to Device Code Flow
...
| Edge rendering | ✅ Chromium-based | Uses standalone Edge browser with SwiftShader software rendering for WinPE compatibility. |
| Redirect capture | ✅ Localhost listener | HTTP listener on random ephemeral port captures the redirect. Traffic never leaves the machine. |
| Graceful fallback | ✅ Transparent | If Edge browser is missing or fails, Device Code Flow is used automatically. |
...
| Phishing risk | ✅ Reduced | Primary flow uses a standalone browser — no codes to copy, no external device needed. Fallback Device Code Flow still has inherent phishing risk (see Finding F-02). |
